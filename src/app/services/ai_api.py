...
class AIApiService:
    ...
    async def start_connection(self):
        ...
        self.response_queue = await self.channel.declare_queue(
            config.rabbitmq.RESPONSE_QUEUE, 
            durable=True
        )
        await self.response_queue.bind(
            self.exchange, 
            routing_key=config.rabbitmq.RESPONSE_QUEUE)
    ...
    async def send_request(
        ...
        logger.info(f"[x] Sent AI API request (corr_id={correlation_id}) via '{routing_key}'")
        ...
        try:
            ...
            logger.warning(f"AI API request (corr_id={correlation_id}) via '{routing_key}' timet out")
